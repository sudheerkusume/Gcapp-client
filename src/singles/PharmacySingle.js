...
const PharmacySingle = () => {
  ...
  useEffect(() => {
    axios.get(`https://gcapp-server.onrender.com/Pharmacy/${id}`)
      .then((res) => 
{
          setProduct(res.data);
...
/>
     ...
     <p className='' style={{font:'bold'}}> Ingredients</p>
    ...
    <div className="col-md-4">
      <h4>{product.title}</h4>
       ...
       <Link to={`/Pharmacy`} className='link-style'>  <h6 className='p-2 text-success mb-lg-5' style={{fontWeight:'smaller'}}>View all by {product.Product} <span className='itspan'>➤</span></h6></Link>
       <div class="price-box ">
